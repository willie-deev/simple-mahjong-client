...
class CardUtils:
	...
	def cardTypeCanPung(self, cardType: CardType):
		if self.gameManager.gotCards.count(cardType) >= 2:
			return True
		return False

	def cardTypeCanKong(self, cardType: CardType):
		if self.gameManager.gotCards.count(cardType) == 3:
			return True
		return False

	def getAllConcealedKong(self) -> list[CardType]:
		concealedKongList: list[CardType] = []
		self.gameManager.gotCards.sort(key=lambda v: v.value)
		lastCard = None
		for card in self.gameManager.gotCards:
			if lastCard == card:
				continue
			lastCard = card
			if self.gameManager.gotCards.count(card) == 4:
				concealedKongList.append(card)
...
def calCanReadyAfterDiscard(cardTypes: list[CardType]) -> list[CardType]:
	canReadyCardTypes: list[CardType] = []
	lastCardType: CardType | None = None
	for cardType in cardTypes:
		if cardType == lastCardType:
			continue
		lastCardType = cardType
		testCardTypes = cardTypes[:]
		testCardTypes.remove(cardType)
		testCardTypes.sort(key=lambda v: v.value)
		if calCanReady(testCardTypes) == 1:
			canReadyCardTypes.append(cardType)
	return canReadyCardTypes
